...
The findings above test each weather variable independently — a valid question
(does this variable's category meaningfully differ in outcome?), but not the
same as asking whether each variable has an *independent* effect once others
are accounted for. A follow-up multivariate Negative Binomial regression,
modeling barometric pressure and "feels-like" temperature together against
both intake and mortality, found that **pressure's apparent effect
disappears once temperature is included** (intake: p=0.311; mortality:
p=0.412), while **temperature remains highly significant for both**
(intake: p<0.001; mortality: p=0.001).

This strongly suggests the univariate pressure findings above were largely
capturing temperature's effect by proxy — storm systems bring both a
pressure drop and a temperature swing together, and the earlier tests
couldn't distinguish which one was actually driving the outcome. Temperature,
not barometric pressure, appears to be the primary real driver of both
intake and mortality. This is a more accurate causal picture than the
original pressure-centric framing, and it's the model actually used in the
downstream [Shelter Overflow Risk Forecaster](https://github.com/MLuftig/shelter-overflow-forecaster).
